...
import { ensureAuthSchema, sql } from "./db.js";
...
app.use(async (_req, _res, next) => {
  try {
    ...
    console.error("[AniloveAuth] schema initialization failed", error);
    next(error);
  }
});
...
async function issueSession(user: { id: string; email: string; role: string }, req: express.Request) {
...
app.get("/health", async (_req, res) => {
  await sql`select 1`;
  await sql`select 1 from public.anilove_auth_users limit 1`;
  res.json({ ok: true, service: "anilove-logs-backend", database: "cockroachdb", authSchema: "ready" });
});
...
app.post("/auth/register", async (req, res) => {
  ...
  if (!email || !password || !username) return res.status(400).json({ error: "Valid email, password, and username are required." });
  ...
  try {
    ...
    if (String(error).includes("unique")) return res.status(409).json({ error: "Email or username is already in use." });
    throw error;
  }
});
...
app.get("/auth/verify-email", async (req, res) => {
  ...
  if (!rows[0]) {
    res.status(400).type("html").send(`<!doctype html><html><body style="margin:0;background:#050507;color:#fff;font-family:Arial;padding:40px 16px"><main style="max-width:520px;margin:auto;padding:32px;background:#0b0b0f;border:1px solid #27272a;border-radius:16px"><p style="color:#f87171;font-weight:bold;letter-spacing:.12em">ANILOVE</p><h1>Verification link expired</h1><p style="color:#a1a1aa;line-height:1.6">Request a new verification email from the Anilove app and try again.</p></main></body></html>`);
    return;
  }
  ...
  res.type("html").send(`<!doctype html><html><body style="margin:0;background:#050507;color:#fff;font-family:Arial;padding:40px 16px"><main style="max-width:520px;margin:auto;padding:32px;background:#0b0b0f;border:1px solid #27272a;border-radius:16px"><p style="color:#a78bfa;font-weight:bold;letter-spacing:.12em">ANILOVE</p><h1>Email verified</h1><p style="color:#d4d4d8;line-height:1.6">Your Anilove account is ready. Return to the app and sign in.</p><p style="display:inline-block;padding:13px 20px;background:#8b5cf6;border-radius:10px;font-weight:bold">Verification complete</p></main></body></html>`);
});
...
app.post("/auth/reset-password", async (req, res) => {
  ...
  if (!rows[0] || !password) return res.status(400).json({ error: "Reset link or password is invalid." });
  const passwordHash = await hashPassword(password);
  await sql.begin(async (transaction) => {
...
app.use((error: unknown, _req: express.Request, res: express.Response, _next: express.NextFunction) => {
  ...
  console.error("[AniloveAuth] request failed", detail);
