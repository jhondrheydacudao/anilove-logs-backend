...
`EMAIL_RELAY_SECRET` as `EXPO_PUBLIC_*` variables.

Email delivery uses the Cloudflare Worker in `cloudflare-worker/`. The Worker
calls Brevo over HTTPS, so the auth backend no longer needs SMTP access.
...
For the auth backend, set `DATABASE_URL` to the CockroachDB connection string with
